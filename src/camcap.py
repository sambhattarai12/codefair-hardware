...
class camcap(threading.Thread):
        ...
        while (self.running):
            ...
            if (file_count >= self.max_files):
                if self.last_max_files_sleep_secs:
                    sleep_secs = min(MAX_FILES_MAX_SLEEP_SECS,
                                     self.last_max_files_sleep_secs * MAX_FILES_BACKOFF_FACTOR)
                else:
                    sleep_secs = MAX_FILES_INITIAL_SLEEP_SECS

                print("camcap:run - {} files in target_dir exceeds maximum of {}. sleeping for {:.2f}s.".format(
                    file_count, self.max_files, sleep_secs))
            ...
            if self.ctl_file and not (os.path.exists(self.ctl_file)):
                print("camcap:run - control file {} not present, sleeping for {:.2f}s".format(self.ctl_file,
                                                                                              CTL_FILE_SLEEP_SECS))
    ...
    def still(self, file):
        ...
        self.camera.exif_tags['GPS.GPSLatitude'] = random.randint(1,1000000000)
